    ...
    public Order createOrderFromCart(Principal principal) {
        ...
        if (user == null) {
            user = userRepository.findAll().stream()
                    ...
                    .findFirst()
                    .orElseThrow(() -> new IllegalStateException("Khong tim thay tai khoan dai ly de tao don hang."));
        }

        Store store = user.getStore();
        if (store == null) {
            throw new IllegalStateException("Tai khoan hien tai chua lien ket cua hang.");
        }
